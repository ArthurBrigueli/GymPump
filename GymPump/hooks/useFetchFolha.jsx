...
const useFetchFolha = (url)=>{
    ...
    const add = async(data)=>{
        ...
        if(user){
            await axios.post('http://192.168.0.102:8080/api/folhas/folha/create', {
                idUser:user.id,
                folha: data
            },{
                headers: {
    ...
    const remove = async()=>{
        ...
        if(user){
            const a = await axios.delete(`http://192.168.0.102:8080/api/folhas/folha/delete/user/${user.id}`,{
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            })
    ...
    useEffect(()=>{
        const fetchData = async()=>{
            ...
            if(user){
                const result = await axios.get(`http://192.168.0.102:8080/api/folhas/user/${user.id}`, {
                    headers: {
                        "Authorization":`Bearer ${token}` 
                    }
                })
