...
const useFetchTreino = (url)=>{
    ...
    const addTreino = async(name, date)=>{
        ...
        if(user.id){
            const idTreino = await axios.post('https://gym-pump-api-fgmo.vercel.app/api/treinos/register', {
                id_usuario: user.id,
                nome: name,
                data: dataFormat,
    ...
    const update = async(id, exercicios)=>{
        if(user.id){
            await axios.put(`https://gym-pump-api-fgmo.vercel.app/api/user/${user.id}/treino/${id}/exercicios/register`, {
    ...
    const updateTreinoId = async(id, nome, data, exercicios) => {
        ...
        if(user.id){
            const result = await axios.put(`https://gym-pump-api-fgmo.vercel.app/api/user/${user.id}/treino/${id}/update`, {
                nome: nome,
                data: data,
                exercicios: exercicios
            })
    ...
    const removeTreinoId = async(id)=>{
        if(user.id){
            await axios.delete(`https://gym-pump-api-fgmo.vercel.app/api/treinos/delete/${user.id}/${id}`)
        }else{
            deleteId(id)
        }
        setCallBack(!callBack)
    ...
    const fetchIdTreino = async(id)=>{
        ...
        if(user.id){
            const result = await axios.get(`https://gym-pump-api-fgmo.vercel.app/api/treino/${user.id}/${id}`)
    ...
    useEffect(()=>{
        setloading(true)
            const fetchData = async()=>{
                ...
                if(user.id){
                    const data = await axios.get(`https://gym-pump-api-fgmo.vercel.app/api/treinos/${user.id}`)
